...
async function GET(req) {
    try {
        ...
        let tickets = dbItems.length > 0
            ? dbItems
            : Object.values(api_auth_1.API_MOCK_SUPPORT_TICKETS);
        // Ensure bidirectional mapping of wireFeed and replies
        tickets = tickets.map(t => {
            const ticketWithReplies = { ...t };
        ...
        if (projectIdFilter) {
            ...
            tickets = tickets.filter(t => t.linkedProjectId?.toLowerCase() === q || t.project_id?.toLowerCase() === q);
        }
        if (assignedToFilter) {
            const q = assignedToFilter.toLowerCase().trim();
            tickets = tickets.filter(t => t.assignedAgent?.toLowerCase() === q || t.assigned_to?.toLowerCase() === q);
        }
        if (searchQuery) {
...
async function POST(req) {
    try {
        ...
        if (!finalSubject?.trim() || !finalDescription?.trim()) {
            return server_1.NextResponse.json({ success: false, error: "Validation Error", message: "Summary subject and description are required." }, { status: 400 });
        }
        const callerClientId = (0, api_auth_1.getClientIdFromAssociation)(caller.clientAssociation);
        const isAdminOrSupport = caller.role === "gff_admin";
        if (!isAdminOrSupport) {
            if (client_id && client_id !== callerClientId) {
                return server_1.NextResponse.json({ success: false, error: "Forbidden", message: "You can only create tickets for your own client account." }, { status: 403 });
            }
        }
        const finalClientId = client_id || callerClientId;
        const client_name = (0, api_auth_1.getClientNameFromId)(finalClientId);
        ...
        api_auth_1.API_MOCK_SUPPORT_TICKETS[newId.toLowerCase()] = newTicket;
        await (0, dynamodb_client_1.dynamoDbPutPortalItem)("SUPPORT", newTicket.client_id, newTicket);
        return server_1.NextResponse.json({ success: true, ticket: newTicket }, { status: 201 });
    }
    catch (err) {
        return server_1.NextResponse.json({ success: false, error: "Internal Server Error" }, { status: 500 });
    }
