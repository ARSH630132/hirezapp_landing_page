...
async function GET(req) {
    try {
        ...
        let users = dynamoUsers.map((user) => ({ ...(0, dynamodb_client_1.mapDynamoUserToApiUser)(user), client_id: String(user.client_id ?? "") }));
...
async function POST(req) {
    try {
        ...
        if (await (0, dynamodb_client_1.getUserFromDynamoDB)(normalizedEmail)) {
