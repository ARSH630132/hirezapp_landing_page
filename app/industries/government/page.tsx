...
const AGENTS = [
  ...
  { name: "Knowledge Search Agent", role: "Indexes regional documentation, records, and procedural handbooks safely." },
...
export default function GovernmentPage() {
  const [demoOutput, setDemoOutput] = useState("System ready. Select a sandbox task below to test GFF Government Solutions.");
  const [running, setRunning] = useState(false);
  const [activeTab, setActiveTab] = useState("gateway");

  const runDemo = (type: "policy" | "case") => {
    setRunning(true);
    if (type === "policy") {
      setDemoOutput("Querying administrative policies database... Matching legislative records...");
      setTimeout(() => {
        setDemoOutput("SUCCESS: Under Local Resolution 442-B, community eligibility matches criteria. Verification logged.");
        setRunning(false);
      }, 600);
    } else {
      setDemoOutput("Parsing inbound case intake form... Extracting applicant metadata...");
      setTimeout(() => {
        setDemoOutput("SUCCESS: Case classified. Routed to Division of Local Housing. Priority: NORMAL. File indexed.");
        setRunning(false);
      }, 600);
    }
  };
  ...
  return (
    <InnerPageShell>
      ...
      <div className="max-w-[1795px] mx-auto px-6 pb-16 grid grid-cols-1 lg:grid-cols-12 gap-8 items-center">
        <div className="lg:col-span-5 flex flex-col justify-center">
          ...
          <p className="text-xs text-white/70 leading-relaxed font-light mb-6">
            Our government-aligned architecture deploys secure ingestion proxies and local sandbox isolation using Control Center and Runtime Governance.
          </p>
          <div className="space-y-2">
            {[
              { id: "gateway", name: "Sovereign Ingestion Gateway", desc: "Isolates incoming case applications and multi-agency records." },
              { id: "control_center", name: "Control Center Orchestrator", desc: "Coordinates Citizen Service Agents and runs policy searches." },
        ...
        <div className="lg:col-span-7 p-6 rounded-2xl border border-white/5 bg-[#050507]/40 backdrop-blur-[12px] flex flex-col items-center">
          <span className="text-[10px] font-mono text-white/40 uppercase tracking-widest mb-4">Interactive Government Architecture Blueprint</span>
          <div className="w-full max-w-[420px] h-[280px] relative">
            <svg className="w-full h-full" viewBox="0 0 500 340" fill="none" xmlns="http://www.w3.org/2000/svg">
              ...
              <g onClick={() => setActiveTab("gateway")} className="cursor-pointer">
                ...
                <text x="250" y="44" fill="white" fontSize="10" fontWeight="bold" textAnchor="middle" className="font-mono">1. SECURE INGESTION GATEWAY</text>
              </g>
              <g onClick={() => setActiveTab("control_center")} className="cursor-pointer">
                ...
                <text x="250" y="134" fill="white" fontSize="10" fontWeight="bold" textAnchor="middle" className="font-mono">2. CONTROL CENTER CORE</text>
              </g>
              <g className="opacity-70">
                <rect x="50" y="190" width="400" height="50" rx="8" fill="#050507" stroke="#009DFF" strokeWidth="1" strokeDasharray="4 4" />
                <text x="250" y="212" fill="#009DFF" fontSize="9" fontWeight="bold" textAnchor="middle" className="font-mono">Citizen Service // Case Triage // Policy Intel</text>
      ...
      <div className="max-w-[1795px] mx-auto px-6 pb-16">
        <h2 className="text-2xl font-bold text-white mb-6">The Government Agent Map</h2>
        <Carousel
          items={AGENTS.map((a) => ({
            title: a.name,
      ...
      <div className="max-w-[1795px] mx-auto px-6 pb-16 grid grid-cols-1 lg:grid-cols-12 gap-8 items-center">
        <div className="lg:col-span-5 flex flex-col justify-center">
          <h2 className="text-2xl font-bold text-white mb-4">Interactive Government Sandbox</h2>
          <p className="text-xs text-white/70 leading-relaxed font-light mb-6">
            Test the Citizen Service and Policy Intelligence modules below. Run an automated legislative policy query or simulate application triaging safely.
          </p>
          <div className="flex flex-wrap gap-3">
            ...
            >
              {running ? "Querying..." : "Query Public Policies"}
            </button>
            <button
              onClick={() => runDemo("case")}
              disabled={running}
            ...
            >
              {running ? "Triaging..." : "Triage Case Intake"}
            </button>
          </div>
        </div>
        ...
        <div className="lg:col-span-7 p-5 rounded-xl border border-white/5 bg-black/40 min-h-[160px] flex flex-col justify-between font-mono relative overflow-hidden">
          <span className="text-[10px] text-[#009DFF] block border-b border-white/5 pb-2 uppercase tracking-wider font-bold font-mono">GOVERNMENT RUNTIME LOGS</span>
          <p className="text-xs text-green-400 mt-3 leading-relaxed font-mono">{demoOutput}</p>
          <span className="text-[9px] text-white/30 text-right block pt-3 font-mono">SECURE SANDBOX // GOV-INT-21</span>
        </div>
      </div>
