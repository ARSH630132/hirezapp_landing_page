...
export default function ClientAnalyticsPage() {
  ...
  if (loading) return (
    <div className="space-y-6 max-w-[1700px] mx-auto pb-16 font-mono p-6 text-white min-h-[400px] flex flex-col justify-center items-center">
      ...
      <div className="text-xs text-white/50 animate-pulse uppercase tracking-widest">Loading dashboard data...</div>
    </div>
  );
  ...
  if (error) return (
    <div className="p-8 text-center max-w-md mx-auto my-16 font-mono text-white border border-red-500/10 bg-red-950/5 rounded-xl">
      ...
      <h3 className="text-sm font-bold uppercase mb-2">CONNECTION ERROR</h3>
  ...
  return (
    <div className="space-y-6 font-mono text-white max-w-[1700px] mx-auto select-none">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center border-b border-white/5 pb-4 gap-4">
        <div>
          <h2 className="text-md font-bold uppercase tracking-wider">Analytics Dashboard</h2>
          <p className="text-[10px] text-white/45 mt-0.5">See real-time system performance, CPU load, and resource stats.</p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 p-4 rounded-xl border border-white/5 bg-[#050505]/40 space-y-4">
          <div className="flex justify-between items-center border-b border-white/5 pb-2">
            <span className="text-xs font-bold uppercase tracking-wider flex items-center gap-2"><Activity className="w-4 h-4 text-[#009DFF]" /> CPU Performance</span>
          ...
          <div className="h-40 bg-black/30 border border-white/5 rounded-xl relative overflow-hidden flex items-end">
            <div className="absolute inset-0 flex flex-col justify-between p-3 opacity-30 text-[8px] text-white/30">
              <div className="w-full border-t border-dashed border-white/5 text-right">95% High</div>
              <div className="w-full border-t border-dashed border-white/5 text-right font-mono">10% Low</div>
            </div>
            ...
            <div className="absolute top-3 left-4 flex gap-4 text-[9px]">
              ...
              <div><span className="text-white/30 block">Highest Load</span><span className="text-[#00FFC2] font-bold text-xs">{maxVal}%</span></div>
            </div>
          </div>
        </div>
        ...
        <div className="p-4 rounded-xl border border-white/5 bg-[#050505]/40 space-y-4 flex flex-col justify-between">
          <div>
            <div className="flex items-center gap-1.5 border-b border-white/5 pb-2 mb-3">
              ...
              <h3 className="text-xs font-bold uppercase tracking-wider">Server Health</h3>
            </div>
          ...
          <div className="bg-black/30 border border-white/5 p-3 rounded-lg text-[9px] text-white/40 leading-relaxed mt-4">
            <span className="text-white font-bold uppercase block mb-1 flex items-center gap-1"><ShieldCheck className="w-3.5 h-3.5 text-[#00FFC2]" /> Secure Memory</span>
            Your computer memory is protected by secure hardware locks. All settings are verified as safe.
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <WorkspaceCard className="border border-white/5 bg-[#050505]/30 p-4 rounded-xl flex flex-col justify-between h-[110px] hover:border-white/10 transition-all">
          <span className="text-[9px] font-bold uppercase tracking-wider flex items-center gap-1.5 text-[#009DFF]"><Layers className="w-3.5 h-3.5" /> Active Projects</span>
          <span className="text-2xl font-bold tracking-tight">{data?.projects?.totalCount || 0}</span>
          <span className="text-[8px] text-white/35">Active Project List: {data?.projects?.byStatus?.active || 0}</span>
        </WorkspaceCard>
        <WorkspaceCard className="border border-white/5 bg-[#050505]/30 p-4 rounded-xl flex flex-col justify-between h-[110px] hover:border-white/10 transition-all">
          <span className="text-[9px] font-bold uppercase tracking-wider flex items-center gap-1.5 text-purple-400"><Cpu className="w-3.5 h-3.5" /> Active AI Agents</span>
          <span className="text-2xl font-bold tracking-tight">{data?.aiOperations?.totalCount || 0}</span>
          <span className="text-[8px] text-white/35">Running: {data?.aiOperations?.byHealth?.healthy || 0}</span>
        </WorkspaceCard>
        <WorkspaceCard className="border border-white/5 bg-[#050505]/30 p-4 rounded-xl flex flex-col justify-between h-[110px] hover:border-white/10 transition-all">
          <span className="text-[9px] font-bold uppercase tracking-wider flex items-center gap-1.5 text-[#00FFC2]"><ShieldCheck className="w-3.5 h-3.5" /> Active Rules</span>
          <span className="text-2xl font-bold tracking-tight">{data?.governance?.totalCount || 0}</span>
          <span className="text-[8px] text-white/35">Standards checked: ISO-27001, SOC2, HIPAA</span>
        </WorkspaceCard>
      </div>

      <div className="p-4 rounded-xl border border-white/5 bg-[#050505]/50 space-y-2">
        <div className="flex justify-between items-center border-b border-white/5 pb-2">
          <span className="text-[9.5px] font-bold uppercase tracking-wider text-white/50">SYSTEM LOG STREAM</span>
          <span className="text-[8.5px] text-white/30 uppercase tracking-widest flex items-center gap-1 font-mono"><span className="w-1.5 h-1.5 bg-[#00FFC2] rounded-full animate-ping" /> Connection Secure</span>
        </div>
      ...
      <div className="flex flex-col md:flex-row justify-between items-center text-[9px] text-white/30 pt-2 border-t border-white/5 gap-2 select-none">
        <span>SYSTEM VERSION: 2026.E3 • LAST SYNC: {lastSync || "NEVER"}</span>
        <span>ALL SYSTEM ACTIVITY LOGGED SECURELY</span>
      </div>
    </div>
  );
}
